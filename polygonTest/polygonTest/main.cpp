...
int WINAPI WinMain(HINSTANCE hInstance, HINSTANCE hInstancePrev, LPSTR pCmdLine, int nCmdShow) {
	...
	// ウィンドウの作成
	WHandle = CreateWindow(ClassName, "３Dポリゴンの描画", WIN_STYLE, CW_USEDEFAULT, CW_USEDEFAULT, 1000, 800, NULL, NULL, hInstance, NULL);
	...
	while (msg.message != WM_QUIT) {
		if (PeekMessage(&msg, NULL, 0, 0, PM_REMOVE)) {
			...
			CONSTANT_BUFFER cb;
			cb.mWVP = XMMatrixTranspose(View * Proj);                               // リソースへ送る値をセット
